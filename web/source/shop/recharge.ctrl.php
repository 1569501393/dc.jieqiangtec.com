...
if($_W['ispost']) {
    ...
    if($pay_type == "baifubao") {
    ...
    }else if($_POST['recharge_type']=='weixinpay'){

		$orderID = $order_no;
        $money = doubleval($_POST['recharge_number']);
        $recharge_type = $_POST['recharge_type'];
        
		//echo $_W['siteroot']."web/source/shop/notify_weixin_url.ctrl.php";
		
        pdo_insert("uni_payorder",
            array(
                "orderid"=>$order_no,
                "money"=>$money,
                "order_time"=>TIMESTAMP,
                "uid"=>$_W['uid'],
                "credittype"=>$recharge_type,
                "pay_type"=>1)
            );
		...
		if(pdo_insertid() > 0) {
            ...
            $input->SetBody("订单号：".$orderID);
            $input->SetAttach($orderID);
            $input->SetOut_trade_no($orderID);
            $input->SetTotal_fee(doubleval($_POST['recharge_number'])*100);//$orderInfo['final_price_cn']
            $input->SetTime_start(date("YmdHis"));
            $input->SetTime_expire(date("YmdHis", time() + 600));
            $input->SetGoods_tag("在线购物");
            
            $input->SetNotify_url($_W['siteroot']."web/source/shop/notify_weixin_url.ctrl.php");
