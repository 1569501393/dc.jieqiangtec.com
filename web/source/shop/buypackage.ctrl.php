...
$usergroups=pdo_fetchall("SELECT * FROM".tablename('users_group') ."where price >= 1 AND  price != 1001 order by price asc",array(':price'=>$group['price']),'id');$ob = Hj\M("ims_users_group_vipprice");$listvipprice = $ob->where(array('ugid'=>$group['id'],'status'=>'1'))->order(array('orderby'=>'asc'))->select();// var_dump($listvipprice);//hjay
	if($_GPC['groupid']){
