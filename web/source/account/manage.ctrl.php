...
if ($do == 'display') {
	...
	$account_lists = uni_account_list($condition, array($pindex, $psize));		$ob = Hj\M("ims_users_group_vipprice");	$listvipprice = $ob->where(array('ugid'=>$group['id'],'status'=>'1'))->order(array('orderby'=>'asc'))->select();		//hjay			$list = $account_lists['list'];
