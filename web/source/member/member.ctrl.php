...
if ($do == 'record') {
		...
		$_W['page']['title'] = "消费记录";
			...
			if($_W['isfounder']){
				...
				$list = pdo_fetchall("SELECT * FROM ".tablename("users_credits_record")." ORDER BY createtime DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
				...
				$list = pdo_fetchall("SELECT * FROM ".tablename("users_credits_record")." WHERE uid=:uid ORDER BY createtime DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize,array(":uid"=>$_W["uid"]));
...
if ($do == 'chongzhi') {
			...
			if($_W['isfounder']){
				...
				$list = pdo_fetchall("SELECT * FROM ".tablename("uni_payorder")." ORDER BY order_time DESC LIMIT ". ($pindex - 1) * $psize . ',' . $psize);
				...
				$list = pdo_fetchall("SELECT * FROM ".tablename("uni_payorder")." WHERE uid=:uid ORDER BY order_time  DESC LIMIT ". ($pindex - 1) * $psize . ',' . $psize,array(":uid"=>$_W["uid"]));
