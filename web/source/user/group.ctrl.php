...
if ($do == 'display') {	uni_user_permission_check('system_user_group');	$_W['page']['title'] = '用户组列表 - 用户组 - 用户管理';
	$condition = '' ;
	$params = array();
	if (!empty($_GPC['name'])) {
	...
	}		$ob2 = Hj\M("ims_users_group_vipprice");	if($_POST['vipprice']=='update'){	    $ob2->where(array('gpid'=>$_POST['gpid']))->save($_POST);	    itoast('更新成功！', url('user/group/display'), 'success');	}	if($_POST['vipprice']=='save'){	    $ob2->add($_POST);	    itoast('添加成功！', url('user/group/display'), 'success');	}	if($_GET['vipprice']=='delete'){	    $ob2->where(array('gpid'=>$_GET['gpid']))->delete();	    itoast('删除成功！', url('user/group/display'), 'success');	}	$pricelist = $ob2->order(array('orderby'=>'asc'))->select();	$ob1 = Hj\M("ims_users_group");	$usergrouplist = $ob1->select();	    // 	$ob1 = Hj\M("ims_users_group");// 	$usergrouplist = $ob1->select();			
	$lists = pdo_fetchall("SELECT * FROM " . tablename('users_group').$condition, $params);	$lists = user_group_format($lists);	//// 	$obcc = Hj\M("ims_users_group_vipprice");										    	    
