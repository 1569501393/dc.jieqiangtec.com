...
class Model {
    ...
    public function __construct($name='',$tablePrefix='',$connection='') {
        ...
        if(is_null($tablePrefix)) {// 前缀为Null表示没有前缀
            ...
            $this->tablePrefix = $this->tablePrefix?$this->tablePrefix:Hj\C('DB_PREFIX');
        }
    ...
    protected function _checkTableInfo() {
        ...
        if(empty($this->fields)) {
            // 如果数据表字段没有定义则自动获取
            if(Hj\C('DB_FIELDS_CACHE')) {
                $db   =  $this->dbName?$this->dbName:Hj\C('DB_NAME');
                $fields = Hj\F('_fields/'.strtolower($db.'.'.$this->name));
                if($fields) {
                    $version    =   Hj\C('DB_FIELD_VERSION');
    ...
    public function flush() {
        ...
        if(Hj\C('DB_FIELD_VERSION')) $this->fields['_version'] =   Hj\C('DB_FIELD_VERSION');

        // 2008-3-7 增加缓存开关控制
        if(Hj\C('DB_FIELDS_CACHE')){
            // 永久缓存数据表信息
            $db   =  $this->dbName?$this->dbName:C('DB_NAME');
            Hj\F('_fields/'.strtolower($db.'.'.$this->name),$this->fields);
        }
    }
     ...
     */
     public function create($data='',$type='') {
        ...
        if(isset($fields)) {
            ...
            // 判断令牌验证字段
            if(Hj\C('TOKEN_ON'))   $fields[] = Hj\C('TOKEN_NAME');
            foreach ($data as $key=>$val){
    ...
    public function autoCheckToken($data) {
        ...
        if(Hj\C('TOKEN_ON')){
            ...
            // 开启TOKEN重置
            if(Hj\C('TOKEN_RESET')) unset($_SESSION[$name][$key]);
            return false;
        }
        return true;
    }
    ...
    protected function parseSql($sql,$parse) {
        ...
        if(true === $parse) {
            ...
            $sql    =   strtr($sql,array('__TABLE__'=>$this->getTableName(),'__PREFIX__'=>Hj\C('DB_PREFIX')));
        }
        $this->db->setModel($this->name);
        return $sql;
    }
    ...
    public function getTableName() {
        if(empty($this->trueTableName)) {
            ...
            if(!empty($this->tableName)) {
                ...
                $tableName .= Hj\parse_name($this->name);
            }
