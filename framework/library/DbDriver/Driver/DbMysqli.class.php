...
class DbMysqli extends Db{
    ...
    public function __construct($config=''){
        if ( !extension_loaded('mysqli') ) {
            throw_exception(Hj\L('_NOT_SUPPERT_').':mysqli');
        }
        if(!empty($config)) {
            $this->config   =   $config;
    ...
    public function connect($config='',$linkNum=0) {
        if ( !isset($this->linkID[$linkNum]) ) {
            ...
            // 设置数据库编码
            $this->linkID[$linkNum]->query("SET NAMES '".Hj\C('DB_CHARSET')."'");
            //设置 sql_model
            if($dbVersion >'5.0.1'){
            ...
            $this->connected    =   true;
            //注销数据库安全信息
            if(1 != Hj\C('DB_DEPLOY_TYPE')) unset($this->config);
        }
